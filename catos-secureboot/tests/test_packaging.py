...
class PackagingTests(unittest.TestCase):
    ...
    def test_package_pins_vendor_boot_chain_and_runtime_tools(self) -> None:
        pkgbuild = (ROOT / "PKGBUILD").read_text(encoding="utf-8")
        self.assertIn("pkgver=0.3.0", pkgbuild)
        self.assertIn("pkgrel=1", pkgbuild)
        self.assertNotIn("pkgver=0.2.0", pkgbuild)
        ...
        self.assertNotIn("'systemd-ukify'", pkgbuild.split("optdepends=", 1)[0])
        self.assertIn("systemd-ukify: build systemd-boot Type #2 UKIs", pkgbuild)
        self.assertIn("catos-firmware-boot: build direct UKIs before shim deployment", pkgbuild)
